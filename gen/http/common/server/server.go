...
// New instantiates HTTP handlers for all the common service endpoints using
...
func (s *Server) Service() string { return "common" }
...
// MountHealthCheckHandler configures the mux to serve the "common" service
...
func MountHealthCheckHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("GET", "/v3/HealthCheck", f)
}

// NewHealthCheckHandler creates a HTTP handler which loads the HTTP request
// and calls the "common" service "health_check" endpoint.
func NewHealthCheckHandler(
	...
	var (
		decodeRequest  = DecodeHealthCheckRequest(mux, decoder)
		encodeResponse = EncodeHealthCheckResponse(encoder)
		encodeError    = EncodeHealthCheckError(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "health_check")
		ctx = context.WithValue(ctx, goa.ServiceKey, "common")
